(ns kotoba.webgl
  "kotoba-webgl — a WebGL2 renderer driven by the SAME EDN as kotoba.webgpu, for browsers without
   WebGPU. It consumes the same render-IR (globals + instances) and the same render-graph (resolved
   through kotoba.gpu for the :webgl2 tier, which drops compute passes), running the GLSL ES 3.00
   shaders that `bb gen-glsl` transpiles from the one EDN shader (kotoba.wgsl → naga → GLSL).
...
(defn pick-backend
  "The best available GPU backend for this browser: :webgpu if WebGPU is present, else :webgl2.
   Both consume the same render-IR; the caller routes to kotoba.webgpu or kotoba.webgl accordingly."
  []
  (if (webgpu-available?) :webgpu :webgl2))

(defn caps
  "The kotoba.gpu capability tier for a running WebGL2 context (no compute / no storage, instancing
...
;; instance layout = kotoba.sprite-gpu/pack-instances: 12 floats — ipos(2) isize(2) irot(1) ishape(1)
...
(defn render-2d!
  "Render a 2D sprite frame on WebGL2: clear, then draw the quad instances (from
   kotoba.sprite-gpu/draw-ops->quads) via the sprite pass. The :sprites pass has no kotoba.gpu
...
;; Mirrors kotoba.webgpu's two-pass lit path on the WebGL2 API. The matrix math (camera→vp, sun→
...
;; color(loc6,vec4) material(loc7,vec4), divisor 1 — 24 f32 / instance, the kotoba.webgpu layout.
...
(defn scene-renderer
  "Build a whole-2D-frame draw fn from the embedded GLSL: a sky gradient pass (fullscreen triangle)
   ...
   draws the full kotoba.scene2d frame on the GPU — the Canvas2D draw-2d! replacement."
  [gl]
  ...
  (let [lit-p (program gl (get-in shaders [:lit :vert]) (get-in shaders [:lit :frag]))
   ...
   (do
     ...
     (defn- browser-only [f data]
       (throw (ex-info (str "kotoba.webgl/" f " is a browser ClojureScript WebGL2 executor")
                       (merge {:namespace 'kotoba.webgl :platform :clj} data))))
